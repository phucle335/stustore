    ...
    if (result.ok) {
      ...
      if (auth.ok) {
        await logAdminAuditAndNotification(
          ...
          {
            type: "admin_action",
            title: "Admin created order",
            body: `Created order ${result.data.id}`,
            entityType: "order",
            entityId: result.data.id,
          },
        );
    ...
    if (result.ok) {
      ...
      if (auth.ok) {
        await logAdminAuditAndNotification(
          ...
          {
            type: "admin_action",
            title: "Admin bulk-deleted orders",
            body: `Bulk-deleted ${result.data.count} orders`,
            entityType: "order",
            entityId: result.data.ids[0] ?? "bulk",
          },
        );
    ...
    if (result.ok) {
      ...
      if (auth.ok) {
        await logAdminAuditAndNotification(
          ...
          {
            type: "admin_action",
            title: "Admin bulk-updated order status",
            body: `Bulk-updated ${result.data.count} orders → ${status}`,
            entityType: "order",
            entityId: result.data.ids[0] ?? "bulk",
          },
        );
