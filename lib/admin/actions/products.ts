      ...
      if (auth.ok) {
        await logAdminAuditAndNotification(
          ...
          },
          {
            ...
            body: `Created product ${result.data.name}`,
            entityType: "product",
            entityId: result.data.id,
          },
        );
      ...
      if (auth.ok) {
        await logAdminAuditAndNotification(
          ...
          },
          {
            type: "admin_action",
            title: "Admin created product",
            body: `Updated product ${result.data.name}`,
            entityType: "product",
            entityId: id,
          },
        );
    ...
    if (result.ok) {
      ...
      if (auth.ok) {
        await logAdminAuditAndNotification(
          ...
          {
            type: "admin_action",
            title: "Admin bulk-deleted products",
            body: `Bulk-deleted ${result.data.count} products`,
            entityType: "product",
            entityId: result.data.ids[0] ?? "bulk",
          },
        );
    ...
    if (result.ok) {
      ...
      if (auth.ok) {
        await logAdminAuditAndNotification(
          ...
          {
            type: "admin_action",
            title: "Admin bulk-updated product status",
            body: `Bulk-updated ${result.data.count} products → ${status}`,
            entityType: "product",
            entityId: result.data.ids[0] ?? "bulk",
          },
        );
