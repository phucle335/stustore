...
export async function finalizePaidOrder({
  ...
  if ((update.data ?? []).length > 0) {
    const finalized = await finalizePaidOrderRow({
      supabaseAdmin,
      order: update.data[0],
      paymentReference: paymentReference ? String(paymentReference) : null,
      paymentPaidAt,
    });
    return { ...finalized, newlyFinalized: true };
  }
  ...
  return { ok: true, order: existing, newlyFinalized: false };
}
...
export async function finalizePaidOrderByReference({
  ...
  return { ok: false, error: "No pending order found for reference", newlyFinalized: false };
}
